//import { messageConstant } from '../../constants/app-constants';
import { FormControl } from '@angular/forms';
import { ErrorMesage, RangeOperation, Operator, CustomErrorType } from './validation-model';
import { FormGroup } from '@angular/forms';

export class CustomValidator {
  static required(control: FormControl, max?: number): ErrorMesage | null {
    ...
    return (control: FormControl): any => {
      ...
      Object.keys(group.controls).forEach(key => {
        if (key !== field) {
          ...
          if (crt && (crt.value as string).trim().toLowerCase() === currentVal.trim().toLowerCase()) {
            isDuplicated = true;
            return;
          }
        }
  ...
  static email(control: FormControl): ErrorMesage | null {
  ...
  static checkSpecialCharacter(customMessage?: string) {
    return (control: FormControl): ErrorMesage | null => {
      const msg = customMessage || "Input can not contain special characters!";
      const value = control.value;
      if (!value) {
        return null;
      } else {
